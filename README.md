...
- Automated Environment Setup: It disables existing agents before initializing a new KeyboardDisplay agent and setting it as the default.
- Controller Configuration: Automatically ensures the Bluetooth controller is powered on and set to both discoverable and pairable modes.
- Interaction Automation: Uses expect to monitor for "yes/no" prompts, "Confirm passkey" requests, and "Authorize service" messages, automatically replying with "yes".
- Automatic Trusting: Detects successful connections via regex, extracts the device MAC address, and issues a trust command to ensure the device can reconnect in the future without intervention.
# Security Note
...
- Listen to the Stream:
  The script will print a URL e.g., http://192.168.1.50:8090/stream.wav.
