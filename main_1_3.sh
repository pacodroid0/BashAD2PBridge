...
function setup_audio_system() {
    ...
    if ! pactl list short sinks | grep -q "$VIRTUAL_SINK_NAME"; then
        pactl load-module module-null-sink sink_name="$VIRTUAL_SINK_NAME" rate=44100 sink_properties=device.description="Lyrion_Stream_Container" > /dev/null
    fi
...
function check_and_install_deps() {
    ...
    local REQUIRED=("ffmpeg" "pulseaudio-utils" "bluez" "rfkill" "expect" "pulseaudio-module-bluetooth" "bc" "lsof" "psmisc")
...
function init_bluetooth() {
    ...
    if [ "$EUID" -ne 0 ]; then sudo rfkill unblock all; else rfkill unblock all; fi
    sleep 1
